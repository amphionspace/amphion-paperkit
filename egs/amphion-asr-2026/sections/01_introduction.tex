...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/capabilities-v3.png}
  \caption{Three production-oriented capabilities of AmphionASR.
    \emph{Top-left:} hotword conditioning via an in-prompt keyword list.
    \emph{Top-right:} target-speaker recognition with a $3$--$5$\,s
    enrollment utterance.
    \emph{Bottom:} degradation robustness via SFT on Voice-in-the-Wild-2M~\cite{xie2026mega}.
    See Sections~\ref{sec:experiments}--\ref{sec:noise} for quantitative results.}
  \label{fig:teaser}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{Real-world acoustic degradations.}
        Production audio routinely contains noise, reverberation,
        far-field attenuation, transmission artefacts and compound
        combinations thereof. The released AmphionASR checkpoint
        addresses this with a dedicated degradation-robustness SFT
        stage trained on Voice-in-the-Wild-2M~\cite{xie2026mega}, a large-scale dataset
        covering diverse real-world degradation conditions.
\end{itemize}
...
target-speaker reference audio, and degraded or overlapping-speaker
...
\begin{itemize}
  \item \textbf{An industrial-grade Mandarin--English SpeechLLM
        ...
        public-plus-internal corpus the model reaches $1.51$\,\%
        CER on AIShell-1 and $1.55$\,\% WER on LibriSpeech
        \texttt{test-clean}, with the full six-Mandarin and
        four-English baseline grid reported in
        Tables~\ref{tab:asr_en}--\ref{tab:asr_zh}
        (Section~\ref{sec:experiments}).
  \item \textbf{Hotword conditioning with scalable retrieval.}
        With five in-prompt hotwords, CommonVoice~17 hotword CER
        drops from $5.93$\,\% to $2.82$\,\% on Mandarin and WER
        from $7.79$\,\% to $5.01$\,\% on English (oracle protocol).
        We further describe a parameter-free text-space
        retrieve-then-bias operator that scales prompt-driven hotword
        behaviour to pools of size up to $10^4$; the retriever
        attains Recall@$K{>}95\%$ on Mandarin at $K=10$ and
        Recall@$K{\approx}80\%$ on English at $K=20$
  ...
  \item \textbf{Target-speaker recognition with reference audio.}
        ...
        internal \texttt{ts\_hw\_test} the released checkpoint
        reaches $12.56$\,\% WER on the target-present
        (\emph{positive}) sub-slice, roughly $4.2\times$ below the
        strongest open-source dual-audio baseline
        (Qwen3-Omni~\cite{xu2025qwen25omni}, $52.90$\,\%); we
        report the LibriMix gap and the residual false-alarm
        behaviour on \emph{negative} sub-slices in
        Section~\ref{sec:tsasr}.
  \item \textbf{A degradation-robustness SFT stage.} On top of the
        capability SFT checkpoint we train a dedicated
        degradation-robustness SFT stage on Voice-in-the-Wild-2M~\cite{xie2026mega},
        a large-scale dataset of real-world degraded speech that
        covers diverse acoustic conditions. The resulting model is
        evaluated on Voices-in-the-Wild-Bench~\cite{xie2026mega}
        against eight publicly reported systems including Mega-ASR,
        Qwen3-ASR-1.7B~\cite{qwen2025qwen3asr}, Kimi-Audio
        \cite{kimiaudio2025} and three closed-source models
...
results and digs into the three flagship capabilities (hotword
conditioning, target-speaker recognition, and degradation robustness);
Section~\ref{sec:analysis} consolidates failure modes and ablations;
we discuss limitations in Section~\ref{sec:limitations} and conclude
