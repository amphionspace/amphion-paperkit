...
\paragraph{Plain-ASR noise curriculum: incidental in v3, controlled
in v3-NR.}
The v3 SFT recipe sees substantial noise / RIR / overlap
augmentation through the TS-ASR branch
(Section~\ref{sec:noise}) but does not apply the same augmentation
to the plain-ASR cuts; the v3-NR variant of
Section~\ref{subsec:v3nr} closes this gap by extending the same
augmentation to the plain-ASR branch and is the checkpoint
reported in Table~\ref{tab:noise_robustness}. Two follow-up items
remain: (i)~the training-data manifest of v3-NR -- which splits
were augmentation-seen versus unseen -- is not yet released in a
machine-readable form, so the unseen-only sub-table of
Section~\ref{subsec:v3nr} cannot yet be assembled; and
(ii)~v3-NR still trails Qwen3-ASR-1.7B \cite{qwen2025qwen3asr} on
ten of the twenty-two degraded rows in
Table~\ref{tab:noise_robustness}, so closing the residual is on
the agenda rather than declared resolved.
% TODO: publish the v3-NR training manifest and rerun the
% Section~\ref{subsec:v3nr} table with training-seen rows marked.
