...
To ensure robust multilingual generalization without overwhelming the emotion-centric tasks, we adopted a conservative sampling strategy for the auxiliary ASR data. The training mixture primarily relied on the proprietary Amphion multilingual in-house dataset, while a limited amount of publicly available ASR data was used as auxiliary semantic-grounding material. Rather than treating these ASR corpora as the main training source, we sampled representative subsets only to broaden acoustic and linguistic coverage. This curated mixture increased acoustic and semantic diversity while keeping the optimization centered on the in-house SER and DEA supervision.
