...
\begin{figure}[!t]
  ...
  \caption{AmphionASR architecture.
    ...
    LLM's embedding space; the text branch tokenises the
...
embeddings directly into the LLM.
...
$2.0$\,B parameters ($1.7$\,B in the LLM
plus $0.3$\,B in the audio encoder).
...
\begin{table}[t]
  ...
  \begin{tabular}{lrl}
    ...
    LLM                         & $1.7$\,B & initialised from Qwen3-ASR-1.7B (28 layers, 2048-d) \\
    Retrieval adapters         & $2.6$\,M & two MLP adapters \\
    \midrule
    Total                      & $2.0$\,B & AuT encoder + LLM + retrieval adapters \\
    \bottomrule
  \end{tabular}
\end{table}
...
thereafter; Stages~2--3 update only the LLM.

\subsection{Language Model}
\label{subsec:llm}

The LLM is initialised from the Qwen3-ASR-1.7B text backbone \cite{qwen2025qwen3asr},
a 28-layer decoder-only Transformer with 2048-dimensional hidden
states and a 64\,K native context length. The LLM is updated during
fine-tuning as described in Section~\ref{sec:training}.
...
This retrieval-and-prompt pattern follows recent contextual-ASR work.
In particular, Kong et al.~\cite{lakomkin2025contextual} already combine
GLCLAP top-$K$ retrieval, prompt injection, and GRPO. Our contribution
boundary is their integration into the shared AmphionASR model and its
broader set of evaluated recognition settings, rather than a new
retrieval or reinforcement-learning principle.
...
\begin{table}[t]
  ...
  \begin{tabular}{lp{9.6cm}}
    ...
    \midrule
    General ASR       & \texttt{Transcribe the following audio.\{speech\}} \\
...
entities without any architectural change. Hotword SFT exposes the
model to the retrieved list as a contextual-bias signal for rare
entities and domain-specific terms. The list carries no dedicated
delimiter; the \verb|Hotwords:| literal is therefore part of the
prompt grammar presented during training.
