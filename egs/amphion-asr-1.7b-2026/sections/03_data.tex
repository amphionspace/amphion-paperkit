...
The sets map directly onto the deployment-facing capabilities of
Section~\ref{sec:intro}: whispered-speech ASR, degradation-robust
ASR, hotword-conditioned ASR, target-speaker ASR (TS-ASR), and
plain transcription training data retained to limit catastrophic
forgetting.
...
plain-transcription corpora in Stage~3. Section~\ref{sec:training}
...
\begin{table}[t]
  ...
  \begin{tabular}{p{4.8cm}lrp{3.2cm}}
    ...
    WhispEar \cite{fang2025whisperear}
      & Whisper ASR     & 222\,440  & bilingual whisper corpus \\
    ...
    AISHELL-2 \cite{du2018aishell2}
      & Plain ASR       & 1\,009\,223 & Mandarin retention \\
    LibriSpeech \cite{panayotov2015librispeech}
      & Plain ASR       & 281\,241  & English retention \\
...
\begin{table}[t]
  ...
  \begin{tabular}{p{4.5cm}p{6.8cm}}
    ...
    AISHELL-2 \cite{du2018aishell2}
      & Plain Mandarin ASR retention ($1.01$\,M) \\
    AISHELL-3 \cite{shi2021aishell3}
      & Hotword-conditioned ASR + target-speaker synthesis \\
    LibriSpeech \cite{panayotov2015librispeech}
      & Plain English ASR retention (train-clean-100/360 + train-other-500) \\
...
\subsection{Plain Transcription Retention}
\label{subsec:plain-asr-retention}

To limit catastrophic forgetting of plain transcription behaviour, the
...
from the mixture---train the model to remain silent when the requested
speaker is absent. The mixing equation, RIR/noise pools, and negative
