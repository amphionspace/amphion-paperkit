...
Both domains expose acoustic conditions that differ from the
clean-speech distribution on which the pretrained encoder was learned.
We therefore unfreeze the audio encoder jointly with the LLM, so that
the encoder adapts its acoustic representations to encode semantic
content under these shifted acoustic conditions, and the two sides
co-adapt before any prompt-driven biasing is introduced.
...
target-speaker partitions (Table~\ref{tab:ts_training_sets}), installing
prompt-driven hotword biasing and target-speaker conditioning on top
of the Stage~1 representation.
...
LibriSpeech~\cite{panayotov2015librispeech}. Replaying the earlier
capability data preserves unconditioned plain-transcription behaviour
while consolidating the hotword and target-speaker capabilities
acquired in Stage~2.
...
After the three-stage SFT recipe we apply a Group Relative Policy
Optimisation (GRPO) pass to the language backbone, keeping the audio
encoder frozen. The sequence-level reward signal targets two failure
modes of teacher-forced SFT decoders that additional SFT epochs do not
directly address: boundary discrimination on silent or
hotword-negative samples, and the metric--train gap between
cross-entropy and CER.
