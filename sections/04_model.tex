...
\begin{figure}[t]
  ...
  \caption{Overview of LLM-ASR. A speech encoder produces frame-level
    ...
    consumed as soft prefix tokens by a pre-trained LLM decoder. The
    encoder is frozen during early training; the adapter is always
    trainable; the LLM is unlocked progressively across the three
    training stages described in Section~\ref{sec:training}.}
  \label{fig:arch}
\end{figure}
