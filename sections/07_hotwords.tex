...
The CommonVoice 17 hotword test splits
(\texttt{commonvoice\_zh\_hotwords}, \texttt{commonvoice\_en\_hotwords})
ship with a per-utterance ground-truth list
\texttt{custom.hotwords} (offline-mined rare entities), filtered by
the QA flag \texttt{custom.clean.pass}. For a target prompt size $k$,
the eval client (\verb|eval_vllm.sh -w |$k$) pads each utterance's
list to $k$ entries by mixing the ground-truth hotwords with random
distractors drawn from the same dataset's hotword pool -- no online
hard negatives are added at evaluation time, in contrast to training
(Section~\ref{subsec:hotword-pool}). The $k=0$ row drops the
\verb|Hotwords:| line entirely (the Train-style prompt instead
renders \verb|Hotwords:N/A|, mirroring the training-time fallback)
and is identical to the corresponding row of
...
\textbf{real terms} (each ground-truth term independently dropped at
$\texttt{miss\_prob}=0.05$), \textbf{up to 10 hard negatives} from a
char-overlap + bigram-Jaccard + pinyin-syllable retriever, and the
\textbf{remainder of $n_\text{distractor}\!\in\![1, 20]$ slots} from
the global pool, with the entire \verb|Hotwords:| line itself kept at
$\texttt{prompt\_hotword\_prob}=0.8$. Training and evaluation sweeps
therefore cover the same prompt-length regime, but evaluation is
strictly easier (no hard-negative pressure).
